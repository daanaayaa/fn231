...
const connection = mysql.createConnection({
    ...
    user: '3cjnyBAKVFtJ5bj.root',
    password:"cYVSm2v4abdliOad",
    database: 'imi_his_db',
    port:4000,
    ssl:{
...
app.post('/addDoctor', urlencodedParser, (req, res) => {
  connection.query('SELECT MAX(doctor_id) as max_id FROM doctor', function(err, result) {
    let nextId = 1;
    if (result[0].max_id) {
      nextId = parseInt(result[0].max_id) + 1;
    }
    let sql = 'INSERT INTO doctor(doctor_id, name, telephone, status) VALUES (?,?,?,?)';
    let values = [nextId, req.body.doctor_name, req.body.doctor_phone, req.body.status];
    connection.query(sql, values, function(err, results, fields) {
      if (err) {
        res.json({ error: true, msg: "Cannot Insert" });
      } else {
